...
float ReadGridFloat::ReverseFloat(const float inFloat)
{
	float retVal;
...
int ReadGridFloat::GetGridFloatFromFile(std::string fileName, std::vector<std::vector<float>> &data, gridFloatData &GFdata) {
	//	Reads in the file responsable for determining how the gridFloat flt file is formatted.
	std::cout << "Reading in files\n";
	if (TerrainVisualizer::fileExists(fileName + ".hdr")) {
		std::ifstream file;
		std::string line;
	...
	*/
	...
	std::cout << "\n";
	if (TerrainVisualizer::fileExists(fileName + ".flt")) {
		std::ifstream file;
